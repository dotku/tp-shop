...
<?php if(($prom_goods != null) OR ($prom_order != null)): ?><section id="search_ka" class="huodong">
		...
		<?php
 $md5_key = md5("select * from `__PREFIX__prom_order` order by  id desc limit 5"); $sql_result_v = S("sql_".$md5_key); if(empty($sql_result_v)) { $Model = new \Think\Model(); $result_name = $sql_result_v = $Model->query("select * from `__PREFIX__prom_order` order by  id desc limit 5"); S("sql_".$md5_key,$sql_result_v,TPSHOP_CACHE_TIME); } foreach($sql_result_v as $k=>$v): ?><ul class="youhui_list1" style="margin-top:0px;">
