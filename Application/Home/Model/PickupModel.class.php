...
class PickupModel extends RelationModel
{
    ...
    public function getPickupItemByPCD($province_id,$city_id,$district_id)
    {
        ...
        $pickup_list = $model
            ...
            ->join('INNER JOIN ' . $db_prefix . 'region AS r1 ON r1.id = p.province_id')
            ->join('INNER JOIN ' . $db_prefix . 'region AS r2 ON r2.id = p.city_id')
            ->join('INNER JOIN ' . $db_prefix . 'region AS r3 ON r3.id = p.district_id')
            ->where($pickup_where)
            ->find();
        return $pickup_list;
    }
